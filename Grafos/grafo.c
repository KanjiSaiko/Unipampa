...
//para cada total da primeira linha lido, ira criar um nodo
...
//
	struct descritor_grafo * insereAresta(struct descritor_grafo *grafo, int chaveSaida, int chaveChegada, int peso){
		...
		//SE O NODO DE SAIDA NAO TIVER ADJACENCIAS (nodos ligados)
		if(nodoSaida->adjacencias == NULL)
			nodoSaida->adjacencias=arestaNova;

		//CASO TIVER, VISITO OS NODOS ATE O PROXIMO FOR NULO, INSIRO LÁ
		else{
...
//BUSCA O NODO/VERTICE DE ACORDO COM A CHAVE
